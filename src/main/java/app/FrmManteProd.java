...
public class FrmManteProd extends JFrame {
	...
	void listado() {
		//Crear un listado de los productos
				...
				for(Producto p: listadoProductos) {
					...
					//con esto, puedes elegir qué mostrar de categoría
					...
					txtSalida.append("Proveedor: " +p.getIdprovedor() + "/"+ p.getProvedor().getNombre_rs()+"\n");
	...
	void buscar() {
		//variables
				...
				cboCategorias.setSelectedIndex(p.getIdcategoria()); //xk está guiado por número "1-Panadol"
																	//si no, sería como getProvedor
				txtPrecio.setText(Double.toString(p.getPrecio()));

				txtStock.setText(Integer.toString(p.getStock()));

				cboProveedores.setSelectedItem(p.getProvedor().getNombre_rs());
